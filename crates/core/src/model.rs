...
impl TrackingFrame {
 ...
 pub fn from_raw_with_config(raw: RawTrackingFrame, config: &TrackingNormalizationConfig) -> Self {
    ...
    let right_eye_pitch_deg = raw.right_eye_pitch_deg.unwrap_or_else(|| raw.eye_pitch_deg.unwrap_or(left_eye_pitch_deg));
  ...
  let eye_delta = (left_eye_yaw_deg - right_eye_yaw_deg).abs().max((left_eye_pitch_deg - right_eye_pitch_deg).abs());
