...
> Status note (2026-04)
>
> この文書は初期フェイズ時点の計画書を履歴として保持しているものです。
> 実装状態と 1:1 で同期する運用は行っていません。
> 現在の正本は README.md / docs/architecture.md / docs/roadmap.md を参照してください。
> 既に実装済みの主な項目: VMC/OSC 受信、TrackIR/NPClient 互換レイヤー、
> GUI からの VMC/OSC ポート変更、ポータブル配布 ZIP 生成スクリプト。
